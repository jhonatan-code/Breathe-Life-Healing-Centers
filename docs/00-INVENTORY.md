...
_Last reviewed: 2026-09-11. Source: `docs/_inbox/` (now sorted into the folders below)._
...
_Update 2026-09-11: fresh 6-month GSC export (`seo/gsc-performance-2026-09-11.xlsx`) reconfirms the blog-decision numbers below are current, not stale — still open._
...
- **GSC performance, refreshed** (`seo/gsc-performance-2026-09-11.xlsx`) — 6 mo: 7,592 clicks / 931k impressions across 415 pages. **Blog/standalone articles alone are 3,344 clicks (44%) and 458k impressions (49%) across 265 URLs** — confirms the 12-mo numbers above aren't a fluke, the same posts are still the traffic engine: `/can-you-have-an-eating-disorder-and-not-realize-it/` (1,297), `/toxic-codependency-looks-like/` (471), `/sex-addiction-drug-addiction/` (201), `/drug-induced-psychosis/` (126), `/mackenzie-phillips-joins-addiction-counselor/` (120), `/vicious-cycle-addiction/` (93), `/5-weird-things-alcohol-withdrawal/` (91), `/hookah-smoking-smoking-tobacco/` (74) — all still uncovered by the redirect map (see item 6). Team bio pages punch above their weight too: 936 clicks / 30.5k impressions across just 48 URLs (3.07% CTR, the best of any page category). Homepage itself is a separate concern: 228k impressions but only 0.84% CTR at position ~17.6 — ranking page 2, barely converting impressions into clicks, worth a look regardless of the blog decision.
...
6. **Approved redirect map received** (`context/Breathe_Life_Redirect_Map_v6_FINAL.xlsx`) — 111 rows + 24 "articles stay live" + status legend. **Page-level coverage is complete and clean**, but three URL classes from the old sitemap have no rule (see Reconciliation below):
   - **~192 of 216 blog Posts** — only 24 eating-disorder articles are explicitly kept live. High-traffic posts have no row: `/toxic-codependency-looks-like/` (935 GSC clicks/12mo), `/sex-addiction-drug-addiction/` (466), `/vicious-cycle-addiction/` (207), `/mackenzie-phillips-joins-breathe-as-addiction-counselor/` (210 + BuzzFeed/VF/Daily Mail backlinks), `/prevent-failure-launch-children/`, `/manage-nicotine-withdrawal/`, `/5-weird-things-alcohol-withdrawal/`, `/drug-induced-psychosis/`, `/hookah-smoking-smoking-tobacco/`. → **needs a blog decision** (keep a blog at same paths = they stay live; kill it = each high-traffic post needs a 301).
     **Reconfirmed 2026-09-11** with a fresher 6-month GSC export (`seo/gsc-performance-2026-09-11.xlsx`) — this class is still ~44% of all organic clicks and ~49% of impressions sitewide (265 URLs), same posts leading. Still unresolved; still not ours to decide. This is not something I'm choosing an answer for — flagging so it doesn't quietly slip through launch.
